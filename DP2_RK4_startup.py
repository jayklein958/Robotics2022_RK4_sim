...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 25 11:34:47 2022
...
class DoublePendulumLagrangian_startup:
    def __init__(self, g, m1, m2, t1,w1,L1, L2,t2a,t2b,kick_time):
        ...
        fixed pivot.
        kt - Time at which the kick occurs
        g - The gravitational acceleration.
        time - global time counter of class
        ...
        t2a - The initial angle of bob #2 before the kick .
        t2b - The final angle of bob #2 after kick.
        N - The number of kicks 
        """
        self.kt = kick_time
        self.g = g
        self.time = 0
    ...
    def time_step(self, dt):
        ...
        if self.kt > self.time:
            self.t2 = self.t2a
        period= (2.767)*1/dt
        N_period = self.N*period
        
        
        if (-1< self.t1 < 1):
            if (self.kt+N_period) < self.time <= ((self.kt+N_period) + (kick_time*1/dt)):
                self.t2 += kick_angle
            elif (self.kt+1222+N_period) < self.time <= ((self.kt+1222+N_period) + (kick_time*1/dt)):
                self.t2 += -kick_angle
                if self.t2 == self.t2a:
                    self.N +=1
